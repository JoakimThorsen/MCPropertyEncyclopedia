...
### Preprocessing
In order to convert the raw Joa Mama output files from output_<type>.json to the <type>_data.json used by the Encyclopedia, a couple of preprocessing and enrichment steps are applied. These are python scripts, and can be found in `processing/`, though they are currently somewhat unstable/WIP.
